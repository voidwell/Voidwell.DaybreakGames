...
namespace Voidwell.DaybreakGames.Services.Planetside
{
    public class MapService : IMapService
    {
        ...
        public async Task<IEnumerable<ZoneRegionOwnership>> GetMapOwnership(int worldId, params int[] zoneId)
        {
            var ownership = await _censusMap.GetMapOwnership(worldId, zoneId);

            return ownership?.SelectMany(a => a.Regions.Row.Select(o => new ZoneRegionOwnership(a.ZoneId, o.RowData.RegionId, o.RowData.FactionId)));
        }
        ...
        public async Task<IEnumerable<ZoneRegionOwnership>> GetMapOwnershipFromHistory(int worldId, int zoneId)
        {
            using (await _zoneHistoryLock.WaitAsync(worldId.ToString()))
            {
                ...
                foreach (var facilityEvent in facilityEvents.OrderBy(a => a.Timestamp))
                {
                    ...
                    regionMapping[region.Id] = new ZoneRegionOwnership(zoneId, region.Id, facilityEvent.FactionNew);
                }
                ...
                foreach (var region in regions)
                {
                    ...
                    {
                        regionMapping.Add(region.Id, new ZoneRegionOwnership(zoneId, region.Id, 0));
                    }
                }
        ...
        public async Task<ZoneSnapshot> GetZoneSnapshotByMetagameEvent(int worldId, int metagameInstanceId)
        {
            ...
            var zoneId = snapshotRegions.First().ZoneId;
            ...
            return new ZoneSnapshot
            {
                ...
                Ownership = snapshotRegions.Select(a => new ZoneRegionOwnership(zoneId, a.RegionId, a.FactionId))
            };
        }
