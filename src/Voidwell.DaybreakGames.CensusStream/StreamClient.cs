...
namespace Voidwell.DaybreakGames.CensusStream
{
    public class StreamClient : IStreamClient
    {
        ...
        private readonly IOptions<DaybreakGamesOptions> _options;
        private readonly ILogger<StreamClient> _logger;

        public StreamClient(IOptions<DaybreakGamesOptions> options, ILogger<StreamClient> logger)
        {
            _options = options;
            _logger = logger;
        }
        ...
        {
            var ns = _options.Value.CensusServiceNamespace;
            var sId = _options.Value.CensusServiceKey;

            return new Uri($"{CensusWebsocketEndpoint}?environment={ns}&service-id=s:{sId}");
        }
